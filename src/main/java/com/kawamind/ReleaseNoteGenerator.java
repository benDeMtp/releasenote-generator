...
public class ReleaseNoteGenerator implements Runnable {
            ...
            versionToDisplay.forEach((td) -> {
                ...
                if(td.types.containsKey("refactor") || td.types.containsKey("perf")){
                    ...
                    if(td.types.containsKey("perf"))
                        td.types.get("perf").stream().forEach(t -> releasenote.println(commitStringAdoc(firstTagReached.get())+t));
                    releasenote.println();
                }
                ...
                if(td.types.containsKey("doc") || td.types.containsKey("docs")){
                    ...
                    if(td.types.containsKey("docs"))
                        td.types.get("docs").stream().forEach(t -> releasenote.println(commitStringAdoc(firstTagReached.get())+t));
                    releasenote.println();
                }
                if(td.types.containsKey("chore")){
                    releasenote.println(typeStringAdoc(firstTagReached.get())+"Divers");
                    td.types.get("chore").stream().forEach(t -> releasenote.println(commitStringAdoc(firstTagReached.get())+t));
                    releasenote.println();
                }
    ...
    Predicate<String> messageFilter = new Predicate<String>() {
        @Override
        public boolean test(String messageToTest) {

            if(Arrays.asList(filteredMessage).stream().anyMatch(m -> messageToTest.contains(m))){
                return false;
            }else{
    ...
    BiFunction<String,String,Boolean> isPoinsonPill = new BiFunction<String,String,Boolean>() {
        @Override
        public Boolean apply(String givenTag, String currentTag) {
